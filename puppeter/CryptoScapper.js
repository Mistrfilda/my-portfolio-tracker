...
export class CryptoScapper extends PuppeteerScraperBase {
	async processEntry(page, entry, index) {
		const { id, name, currency } = entry;

		const selectors = [
			"::-p-xpath(/html/body/div[2]/div[3]/main/section/section/section/section/section[1]/div/div[3]/div/table)",
		];
		...
		if (!element) {
			console.error(`Failed to find crypto table for ${name} with any selector`);
			return null;
		}
